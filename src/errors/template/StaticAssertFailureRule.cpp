/**
 *
 *  @file StaticAssertFailureRule.cpp
...
namespace vix::cli::errors::template_rules
{
  namespace
  {
    ...
    {
      (void)message;
      return "read the static_assert message and check the compile-time condition";
    }
  } // namespace

  class StaticAssertFailureRule final : public ITemplateErrorRule
  {
  public:
    bool match(const vix::cli::errors::CompilerError &err) const override
    {
      ...
      if (icontains(message, "static assertion failed"))
        return true;

      if (icontains(message, "static_assert failed"))
        return true;
  ...
  {
    return std::make_unique<StaticAssertFailureRule>();
  }
} // namespace vix::cli::errors::template_rules
