...
namespace vix::cli::make::generators
{
  ...
  namespace
  {
                                                   ...
                                                   const fs::path &relative_header_path)
    {
      ...
      out << "  std::int32_t version = 1;\n";
      out << "  std::vector<std::string> tags;\n";
      out << "};\n";
  ...
  MakeResult generate_struct(const MakeContext &ctx)
  {
    MakeResult result;
    ...
    result.notes.push_back("Generated plain data struct.");
    result.notes.push_back("Struct generation is header-only by design.");
    result.notes.push_back("Useful for DTOs, configs, payloads, and transport objects.");
