...
namespace vix::cli::errors::template_rules
{
  namespace
  {
    ...
    {
      (void)message;
      return "do not use the result of a void function as a value";
    }
  } // namespace

  class InvalidUseOfVoidRule final : public ITemplateErrorRule
  {
  public:
    bool match(const vix::cli::errors::CompilerError &err) const override
    {
      ...
      if (icontains(message, "invalid use of void expression"))
        return true;

      if (icontains(message, "void value not ignored"))
        return true;

      if (icontains(message, "forming reference to void"))
        return true;

      if (icontains(message, "variable or field") &&
          icontains(message, "declared void"))
      {
        return true;
      }
  ...
  {
    return std::make_unique<InvalidUseOfVoidRule>();
  }
} // namespace vix::cli::errors::template_rules
