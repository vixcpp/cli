...
namespace vix::cli::errors::template_rules
{
  namespace
  {
    ...
    {
      (void)message;
      return "mark the member function const or avoid modifying data through "
             "a const object";
    }
  } // namespace

  class ConstQualifierRule final : public ITemplateErrorRule
  {
  public:
    bool match(const vix::cli::errors::CompilerError &err) const override
    {
      ...
      if (icontains(message, "passing") &&
          icontains(message, "const") &&
          icontains(message, "discards qualifiers"))
      {
        return true;
      }

      if (icontains(message, "discards qualifiers"))
        return true;

      if (icontains(message, "this argument has type") &&
          icontains(message, "const"))
      {
        return true;
      }
      ...
      if (icontains(message, "assignment of member") &&
          icontains(message, "read-only"))
      {
        return true;
      }

      if (icontains(message, "read-only variable") ||
          icontains(message, "read-only location"))
      {
        return true;
      }
    ...
    {
      std::cerr << RED
                << "error: const qualifier mismatch"
                << RESET << "\n";
  ...
  {
    return std::make_unique<ConstQualifierRule>();
  }
} // namespace vix::cli::errors::template_rules
