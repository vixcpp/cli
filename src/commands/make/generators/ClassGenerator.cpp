...
namespace vix::cli::make::generators
{
  ...
  namespace
  {
                                                   ...
                                                   const fs::path &relative_header_path)
    {
      ...
      out << "public:\n";
      out << "  " << ctx.name << "();\n";
      out << "  explicit " << ctx.name << "(std::string id);\n";
      out << "  virtual ~" << ctx.name << "() = default;\n\n";

      out << "  " << ctx.name << "(const " << ctx.name << " &) = default;\n";
      out << "  " << ctx.name << "(" << ctx.name << " &&) noexcept = default;\n";
      out << "  " << ctx.name << " &operator=(const " << ctx.name << " &) = default;\n";
      out << "  " << ctx.name << " &operator=(" << ctx.name << " &&) noexcept = default;\n\n";

      out << "  [[nodiscard]] const std::string &id() const noexcept;\n";
      out << "  void set_id(std::string value);\n\n";
                                                   ...
                                                   const fs::path &relative_header_path)
    {
      ...
      out << qname << "::" << ctx.name << "() = default;\n\n";

      out << qname << "::" << ctx.name << "(std::string id)\n";
      out << "    : id_(std::move(id))\n";
      out << "{\n";
      out << "}\n\n";

      out << "const std::string &" << qname << "::id() const noexcept\n";
      out << "{\n";
      out << "  return id_;\n";
      out << "}\n\n";

      out << "void " << qname << "::set_id(std::string value)\n";
      out << "{\n";
      out << "  id_ = std::move(value);\n";
      out << "}\n";
  ...
  MakeResult generate_class(const MakeContext &ctx)
  {
    ...
    std::error_code ec{};
    const fs::path relative_header_path = fs::relative(header_path, ctx.layout.root, ec);
    ...
    result.notes.push_back("Generated modern class skeleton.");
