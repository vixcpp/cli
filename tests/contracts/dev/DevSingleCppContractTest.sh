...
[[ $(grep -c 'changed:' "$LOG") -eq 1 ]] || fail 'one b.hpp write was accepted more than once'

# No input change remains idle.  A direct header is a dependency too.
sleep 1
[[ $(grep -c 'changed:' "$LOG") -eq 1 ]] || fail 'idle watcher rebuilt unexpectedly'
...
touch_before=$(grep -c 'changed:' "$LOG")
touch "$ROOT/b.hpp"
for _ in $(seq 1 100); do [[ $(grep -c 'changed:' "$LOG") -eq $((touch_before + 1)) ]] && break; sleep .1; done
[[ $(grep -c 'changed:' "$LOG") -eq $((touch_before + 1)) ]] || fail 'touch-only dependency change was not observed'
sleep 1
[[ $(grep -c 'changed:' "$LOG") -eq $((touch_before + 1)) ]] || fail 'touch-only dependency change looped'
...
sleep 1
before_stale=$(grep -c 'changed:' "$LOG")
printf '%s\n' '#pragma once' '#include "b.hpp"' '#define VALUE (B_VALUE + 11)' > "$ROOT/a.hpp"
sleep 1.5
[[ $(grep -c 'changed:' "$LOG") -eq "$before_stale" ]] || fail 'stale a.hpp remained watched after include graph change'
