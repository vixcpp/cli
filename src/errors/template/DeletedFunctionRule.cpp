...
namespace vix::cli::errors::template_rules
{
  namespace
  {
    ...
    {
      (void)message;
      return "check whether the type is non-copyable, non-movable, "
             "or has a deleted constructor/operator";
    }
  } // namespace

  class DeletedFunctionRule final : public ITemplateErrorRule
  {
  public:
    bool match(const vix::cli::errors::CompilerError &err) const override
    {
      ...
      if (icontains(message, "call to deleted constructor"))
        return true;

      if (icontains(message, "implicitly deleted"))
        return true;

      // Generic "is deleted" needs more context to avoid false positives
      // (e.g. UB log mentioning "memory is deleted"). Require typical compile signals.
      if (icontains(message, "is deleted") &&
          (icontains(message, "function") ||
  ...
  {
    return std::make_unique<DeletedFunctionRule>();
  }
} // namespace vix::cli::errors::template_rules
