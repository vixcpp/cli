/**
 *
 *  @file NarrowingConversionRule.cpp
...
namespace vix::cli::errors::template_rules
{
  namespace
  {
    ...
    {
      (void)message;
      return "use an explicit cast only if the loss of precision is intentional";
    }
  } // namespace

  class NarrowingConversionRule final : public ITemplateErrorRule
  {
  public:
    bool match(const vix::cli::errors::CompilerError &err) const override
    {
      ...
      if (icontains(message, "non-constant-expression cannot be narrowed"))
        return true;

      if (icontains(message, "changes value from"))
        return true;
  ...
  {
    return std::make_unique<NarrowingConversionRule>();
  }
} // namespace vix::cli::errors::template_rules
