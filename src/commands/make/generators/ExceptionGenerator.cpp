...
namespace vix::cli::make::generators
{
  ...
  namespace
  {
                                                   ...
                                                   const fs::path &relative_header_path)
    {
      ...
      out << "class " << ctx.name << " final : public std::exception\n";
      out << "{\n";
      out << "public:\n";
      out << "  explicit " << ctx.name << "(std::string message)";
      ...
      if (ctx.options.header_only)
      {
        out << "      : message_(std::move(message))\n";
        out << "  {\n";
        out << "  }\n\n";
      }

      out << "  [[nodiscard]] const char *what() const noexcept override";
                                                   ...
                                                   const fs::path &relative_header_path)
    {
      ...
      out << qname << "::" << ctx.name << "(std::string message)\n";
      out << "    : message_(std::move(message))\n";
      out << "{\n";
      out << "}\n\n";

      out << "const char *" << qname << "::what() const noexcept\n";
      out << "{\n";
      out << "  return message_.c_str();\n";
      out << "}\n";
  ...
  MakeResult generate_exception(const MakeContext &ctx)
  {
    MakeResult result;
    ...
    result.notes.push_back("Generated std::exception derived type.");
    result.notes.push_back("Stores the error message in std::string.");
