...
int main()
{
  ...
  assert(value.requirements.size() == 6); // root registry + root git + auth(2) + billing + active space; disabled missing is retained only as graph node
  assert(value.links.size() == 5);
