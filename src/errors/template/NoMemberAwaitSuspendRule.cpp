/**
 *
 *  @file NoMemberAwaitSuspendRule.cpp
...
namespace vix::cli::errors::template_rules
{
  ...
  class NoMemberAwaitSuspendRule final : public ITemplateErrorRule
  {
  public:
    bool match(const vix::cli::errors::CompilerError &err) const override
    {
      ...
      return (icontains(m, "await_suspend") &&
        ...
        const vix::cli::errors::ErrorContext &ctx) const override
    {
      ...
      std::cerr << RED
                << "error: "
                << RESET
                << "missing await_suspend in awaiter"
                << "\n";
      ...
      std::cerr << YELLOW
                << "hint: "
                << RESET
                << "the object used with co_await does not provide await_suspend() as part of the awaiter protocol"
                << "\n";

      std::cerr << YELLOW
                << "hint: "
                << RESET
                << "define await_suspend() with a valid signature or provide operator co_await that returns a valid awaiter type"
                << "\n";
  ...
  {
    return std::make_unique<NoMemberAwaitSuspendRule>();
  }
} // namespace vix::cli::errors::template_rules
