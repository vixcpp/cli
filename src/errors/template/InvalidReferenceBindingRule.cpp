/**
 *
 *  @file InvalidReferenceBindingRule.cpp
...
namespace vix::cli::errors::template_rules
{
  namespace
  {
    ...
    {
      (void)message;
      return "use const reference for temporaries, pass an lvalue, "
             "or change the parameter type";
    }
  } // namespace

  class InvalidReferenceBindingRule final : public ITemplateErrorRule
  {
  public:
    bool match(const vix::cli::errors::CompilerError &err) const override
    {
      ...
      if (icontains(message, "cannot bind non-const lvalue reference"))
        return true;

      if (icontains(message, "cannot bind rvalue"))
        return true;

      if (icontains(message, "binds to a temporary"))
        return true;

      if (icontains(message, "invalid initialization of non-const reference"))
        return true;

      if (icontains(message, "non-const lvalue reference") &&
    ...
    {
      std::cerr << RED
                << "error: invalid reference binding"
                << RESET << "\n";
  ...
  {
    return std::make_unique<InvalidReferenceBindingRule>();
  }
} // namespace vix::cli::errors::template_rules
