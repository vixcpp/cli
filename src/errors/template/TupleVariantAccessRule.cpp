/**
 *
 *  @file TupleVariantAccessRule.cpp
...
namespace vix::cli::errors::template_rules
{
  namespace
  {
    ...
    {
      (void)message;
      return "check the index or type used with std::get";
    }
  } // namespace

  class TupleVariantAccessRule final : public ITemplateErrorRule
  {
  public:
    bool match(const vix::cli::errors::CompilerError &err) const override
    {
      ...
      if (icontains(message, "tuple index must be in range"))
        return true;

      if (icontains(message, "tuple_element"))
        return true;

      if (icontains(message, "variant index"))
        return true;

      if (icontains(message, "bad_variant_access"))
        return true;

      // "std::get" alone is too generic; require an additional access-style signal.
      if (icontains(message, "std::get") &&
          (icontains(message, "no matching") ||
           icontains(message, "not unique") ||
           icontains(message, "not exactly once") ||
           icontains(message, "ambiguous")))
      {
        return true;
      }
    ...
    {
      std::cerr << RED
                << "error: invalid tuple/variant access"
                << RESET << "\n";
  ...
  {
    return std::make_unique<TupleVariantAccessRule>();
  }
} // namespace vix::cli::errors::template_rules
