/**
 *
 *  @file NoViableConversionRule.cpp
...
namespace vix::cli::errors::template_rules
{
  namespace
  {
    ...
    {
      (void)message;
      return "check constructor overloads, explicit constructors, return type, "
             "and argument types";
    }
  } // namespace

  class NoViableConversionRule final : public ITemplateErrorRule
  {
  public:
    bool match(const vix::cli::errors::CompilerError &err) const override
    {
      ...
      // Avoid stealing narrowing-conversion diagnostics handled elsewhere.
      if (icontains(message, "narrowing conversion"))
        return false;

      if (icontains(message, "no viable conversion"))
        return true;

      if (icontains(message, "could not convert"))
        return true;

      if (icontains(message, "cannot convert"))
        return true;

      if (icontains(message, "conversion from") &&
          icontains(message, "to non-scalar type requested"))
      {
        return true;
      }
  ...
  {
    return std::make_unique<NoViableConversionRule>();
  }
} // namespace vix::cli::errors::template_rules
