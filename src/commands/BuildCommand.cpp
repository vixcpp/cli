...
namespace vix::commands::BuildCommand
{
  namespace
  {
        ...
        const fs::path &sdkConfigDir)
    {
      ...
      // CMAKE_PROJECT_INCLUDE runs as the final step of project(), after
      // languages are enabled.  The bridge guards nested project() calls,
      // while keeping the user's CMakeLists.txt entirely untouched.
      if (vixDepsLoader)
      {
        // Migrate only the legacy value written by Vix.  Do not disturb a
        // caller's unrelated top-level include configuration.
        if (clearLegacyVixTopLevelInclude)
          variables.emplace_back("CMAKE_PROJECT_TOP_LEVEL_INCLUDES", "");
        variables.emplace_back(
            "CMAKE_PROJECT_INCLUDE",
            vixDepsLoader->string());
      }

      return variables;
    }

    static std::optional<fs::path> cmake_vix_deps_loader(
        const fs::path &projectDir,
        ...
        std::string &errorMessage)
    {
      ...
      const fs::path manifestPath = projectDir / "vix.app";
      if (!util::file_exists(manifestPath))
        return std::nullopt;

      const auto load = app::load_app_manifest(
          manifestPath,
          app::AppManifestLoadMode::DependenciesOnly);
      if (!load.success())
      {
        errorMessage = "Failed to load dependency manifest vix.app: " + load.error;
        return std::nullopt;
      }

      if (load.manifest.deps.empty() && load.manifest.gitDependencies.empty())
        return std::nullopt;

      return fs::absolute(projectDir / ".vix" / "vix_deps.cmake").lexically_normal();
    }

    static std::optional<fs::path> write_cmake_vix_deps_project_bridge(
        const std::optional<fs::path> &depsLoader,
        const fs::path &buildDir,
        std::string &errorMessage)
    {
      if (!depsLoader)
      ...
      const fs::path bridge = buildDir / "vix-project-deps.cmake";
      std::ostringstream content;
      content << "# Generated by Vix. Loaded by CMAKE_PROJECT_INCLUDE.\n";
      content << "get_property(_vix_project_deps_loaded GLOBAL PROPERTY VIX_PROJECT_DEPS_LOADED)\n";
      content << "if(NOT _vix_project_deps_loaded)\n";
      content << "  set_property(GLOBAL PROPERTY VIX_PROJECT_DEPS_LOADED TRUE)\n";
      content << "  include([==[" << depsLoader->generic_string() << "]==])\n";
      content << "endif()\n";

      if (!write_if_different(bridge, content.str()))
      {
        errorMessage = "Failed to generate CMake dependency bridge: " + bridge.string();
        return std::nullopt;
      }
    ...
    static bool has_legacy_vix_top_level_include(
        const fs::path &buildDir,
        const std::optional<fs::path> &depsLoader)
    {
      if (!depsLoader)
      ...
      std::istringstream cache(
          util::read_text_file_or_empty(buildDir / "CMakeCache.txt"));
      const std::string prefix = "CMAKE_PROJECT_TOP_LEVEL_INCLUDES:";
      const std::string expected = depsLoader->string();
      std::string line;
        ...
        const fs::path &cwd)
    {
      ...
      const auto vixDepsLoader = cmake_vix_deps_loader(
          userProjectDir, hasCMakeLists, dependencyManifestError);
      ...
      const bool clearLegacyVixTopLevelInclude =
          has_legacy_vix_top_level_include(plan.buildDir, vixDepsLoader);
