...
namespace vix::cli::errors::template_rules
{
  namespace
  {
    ...
    {
      (void)message;
      return "define the operator, use the correct operand types, "
             "or call an explicit function instead";
    }
  } // namespace

  class OperatorNotFoundRule final : public ITemplateErrorRule
  {
  public:
    bool match(const vix::cli::errors::CompilerError &err) const override
    {
      ...
      if (icontains(message, "no match for") &&
          icontains(message, "operator"))
      {
        return true;
      }

      if (icontains(message, "invalid operands to binary expression"))
        return true;

      if (icontains(message, "invalid operands of types"))
        return true;

      if (icontains(message, "no match for 'operator"))
        return true;

      if (icontains(message, "cannot use operator"))
        return true;
  ...
  {
    return std::make_unique<OperatorNotFoundRule>();
  }
} // namespace vix::cli::errors::template_rules
